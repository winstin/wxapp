...
import { View, ScrollView, Swiper, Image, SwiperItem } from "@tarojs/components";
import { AtDrawer, AtSearchBar, AtMessage } from 'taro-ui'
...
@connect(({ factory, global, loading }) => {
  const { userInfo = {}, corporateData, imgUrls } = factory;
...
class Home extends Component {
  ...
  fetchListBg = (page = 1) => {
    ...
    if (dispatch) {
      ...
      dispatch({
        type: "factory/getCorporateBgList"
      }).then((e) => {
        console.log(e)
        this.setState({
          imgUrls: e
  ...
  render() {
    const { sort, industry, corporateData, industryObject, sortObject, imgUrls } = this.state;
    console.log(imgUrls);
            ...
            {imgUrls.map((item) => {
              return (
                <SwiperItem onClick={() => {
                  if (isLogined()) return;
                  Taro.navigateTo({
                    url: `/pages/FactoryDetail/index?id=${item.id}`
                  })
                }}>
                  <Image src={`http://sz-spd.cn:889/${item.photoCover}`} className={styles.SwiperItem} />
                </SwiperItem>
              )
            })}
