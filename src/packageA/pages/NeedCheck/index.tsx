...
class Home extends Component {
  ...
  submit = (type) =>{
    const {dispatch} = this.props;
    const {reqDesc,qty,itemName,drawings,selectorChecked,id} = this.props.jxhReqDetail;
    ...
    if(dispatch){
      dispatch({
        ...
        payload: {
          ...
          drawings,
          isTop:"1",
          isShowDrawing:selectorChecked==="显示"?'1':'0'
        }
      }).then(()=>{
        Taro.showToast({
          'title': '操作成功',
  ...
  render() {
    ...
    const {reqDesc,qty,itemName,createdDate,drawings,reqName} = this.props.jxhReqDetail;
    return (
